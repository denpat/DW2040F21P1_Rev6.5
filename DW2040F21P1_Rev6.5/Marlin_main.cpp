...
Modified by Dennis Patella WASPROJECT TEAM from 10/2015 
...
// G28 - Home all Axis
// G29 - Detailed Z-Probe, probes the bed at 4 points.  Will fail if you haven't homed yet.
...
// M106 - Fan on M106 Sxxx set fan to xx Lxxx set MIN FAN to XX
// M107 - Fan off
...
// M665 - set delta configurations L[diagonal rod] R[radius] Z[Zmax] A[dA] B[dB] C[dC] M[max plater radius]
...
void setup_ext_pin()
{
    ...
    WRITE(EXT_FILAMENT,HIGH);
  #endif
}
void setup_killpin()
{
  #if defined(KILL_PIN) && KILL_PIN > -1
    SET_INPUT(KILL_PIN,INPUT);
    WRITE(KILL_PIN,HIGH);
  #endif
}
...
void setup()
{
  ...
  SERIAL_ECHO("File: ");
  SERIAL_ECHO(resurrectionData.nameDir);
  SERIAL_ECHO("/");
  SERIAL_ECHOLN(resurrectionData.nameFile);
  SERIAL_ECHO("Pos: Z: ");
  ...
  SERIAL_ECHO(" Bed: ");
  SERIAL_ECHO(resurrectionData.bed);
  SERIAL_ECHO(" Extruder: ");
  SERIAL_ECHOLN(resurrectionData.extruder);
  SERIAL_ECHO("SDPOS: ");
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
	...
	if (Stopped == false) {
        ...
        get_coordinates(); // For X Y Z E F
		  ...
		  if (code_seen('Z')) precCurrentZ=destination[Z_AXIS];
		  if ((code_seen('X') || code_seen('Y')) && (card.sdprinting) && (current_position[Z_AXIS] < 0.5) && (precCurrentZ <= current_position[Z_AXIS])) 
		  {
			  isFirstLayer=true;
			  //if (babystepsTodo[Z_AXIS]==0) babystepsTodo[Z_AXIS] = (int)((axis_steps_per_unit[Z_AXIS]*current_position[Z_AXIS]/2)) * -1; 
    ...
    case 29:         
	{
		...
		autoLevel=true;
		if (clayMode) {
				LCD_MESSAGEPGM("! EXIT LDM !");
				break;	
			}
			int preheatTemp = 190; // Preheat extruder temperature 
			float pointX[3] = {0.0,-50.0,50.0};  //x1 x2 x3
			float pointY[3] = {-70.0,50.0,50.0}; //y1 y2 y3
			float pointZ[3] = {0.0,0.0,0.0};  //z1 z2 z3
			float curZ = MANUAL_Z_HOME_POS + ZcorNoClay;
			float TP1,TP2,TP3;	//test point result
			float tolerance = 0.1; 	
			...
			LCD_MESSAGEPGM("Extruder heating");
			setTargetHotend(preheatTemp, tmp_extruder);
			if (int(degHotend(0) + 0.5) < preheatTemp ) {  //PREHEAT_TEMP
				setTargetHotend(preheatTemp,active_extruder);
			...
			{
			destination[X_AXIS]=pointX[0];
			destination[Y_AXIS]=pointY[0];
			destination[Z_AXIS]=pointZ[0];
			prepare_move();
			actualTP=1;
			for(;;) {
				prepare_move();
			if(gotoTP==0) {
			LCD_MESSAGEPGM("  <<<< MOVE UP <<<< ");
				
			}
			if(gotoTP==actualTP) 
			LCD_MESSAGEPGM(" >>>> MOVE DOWN >>>>");;
			
			if(gotoTP==actualTP+1)
			break;
			manage_heater();
		
			}
			LCD_MESSAGEPGM("  Test Point 1 OK  ");
			delay(1000);
			destination[X_AXIS]=pointX[1];
			destination[Y_AXIS]=pointY[1];
			destination[Z_AXIS]=pointZ[1];
			prepare_move();
			actualTP=gotoTP;
			for(;;) {
			if(gotoTP==0) {
			...
			LCD_MESSAGEPGM("  Test Point 2 OK  ");
			delay(1000);
			destination[X_AXIS]=pointX[2];
			destination[Y_AXIS]=pointY[2];
			destination[Z_AXIS]=pointZ[2];
			prepare_move();
			actualTP=gotoTP;
			for(;;) {
			if(gotoTP==0) {
			...
			LCD_MESSAGEPGM("  Test Point 3 OK  ");
			delay(1000);
			}
			/*
			...
			add_homeing[2]=0.0;
			float initial_mm = st_get_position_mm(Z_AXIS);
			float final_mm = initial_mm;
			float delta_mm = 0.0;
		...
		//X=0 Y=0 Z=0 touching buildplate
		
		
          
          plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], feedrate/240, active_extruder);
          st_synchronize();
          endstops_hit_on_purpose();
		  final_mm = st_get_position_mm(Z_AXIS);
		  for(int8_t i=0; i < NUM_AXIS; i++) {
		...
		delta_mm = (initial_mm - final_mm)+tolerance;
		char status[40];
		strcpy(status,"New Zmax");
		strcat(status,"=");
		strcat(status,ftostr32(delta_mm));
		
		//LCD_MESSAGEPGM(status);
		
		
		
		ZcorNoClay=(MANUAL_Z_HOME_POS - delta_mm )*-1.0;
		...
		//
		for(;;) {
			if (lcd_clicked()) break;
			delay(500);	
				
			TP1=probeZ();
			
			 if ((TP1<=2*tolerance)&&(TP1>0)) {
				 LCD_MESSAGEPGM("  Test Point 1 OK  ");
				 lcd_update();
				 break;
			...
			{
			if (TP1>2*tolerance) LCD_MESSAGEPGM(" >>>> MOVE DOWN >>>>");
			else if (TP1<0) LCD_MESSAGEPGM("  <<<< MOVE UP <<<< ");
			}
			lcd_update();
			//strcat(status,ftostr32(TP1));
			//LCD_ALERTMESSAGEPGM(status);
			//lcd_reset_alert_level();
			delay(500);
		}
		//lcd_reset_alert_level();
		...
		for(;;) {
			...
			if (TP2>0.2) LCD_MESSAGEPGM(" >>>> MOVE DOWN >>>>");
			else if (TP2<0) LCD_MESSAGEPGM("  <<<< MOVE UP <<<< ");
			}
			lcd_update();
			delay(1000);
		} 
		...
		for(;;) {
			...
			if (TP3>0.2) LCD_MESSAGEPGM(" >>>> MOVE DOWN >>>>");
			else if (TP3<0) LCD_MESSAGEPGM("  <<<< MOVE UP <<<< ");
			}
			lcd_update();
			delay(1000);
		} 
  ...
  else if(code_seen('M'))
  {
	...
	case 2:
		...
		resurrectionData.extruder = degTargetHotend(tmp_extruder);
    ...
    case 23: //M23 - Select file
      ...
      if(starpos!=NULL)
        *(starpos)='\0';
		...
		memcpy(resurrectionData.nameFile,filePrinting,strlen(filePrinting));
		resurrectionData.nameFile[strlen(filePrinting)]=0;
...
if (!clayMode) {
	...
	//retract by E
        ...
        while(!lcd_clicked()){
          cnt++;
		if (!clayMode) {
		if ((millis() - start_timeout_preheat) > (uint32_t)TIMEOUT_PREHEAT) {
...
LCD_ALERTMESSAGEPGM(MSG_OUTOFPLATE);
outOfPlate=false;
...
if (((millis() - start_timeout_preheat) > (uint32_t)TIMEOUT_PREHEAT) && !card.sdprinting ) {
...
#endif
#if defined(EXT_FILAMENT) && EXT_FILAMENT > -1
bool state_ext_fil = READ(EXT_FILAMENT);
...
void resurrecting() {
	...
	resurState=true;
	card.initsd();	
	LCD_ALERTMESSAGEPGM(" Resurrecting...");
	
	Estopped=false;
	firstMove=true;
	if (!clayMode) {
	setTargetBed((int)resurrectionData.bed);
	if (int(degHotend(1) + 0.5)<PREHEAT_TEMP ) {
		setTargetHotend(PREHEAT_TEMP,active_extruder);
		while(isHeatingHotend(active_extruder)) {
			manage_heater();
			lcd_update();
		}	
...
endstops_hit_on_purpose();
	delay(1000);
	if (!clayMode) {
	if (int(degHotend(1) + 0.5)<(int)resurrectionData.extruder ) {
		setTargetHotend((int)resurrectionData.extruder,active_extruder);
		while(isHeatingHotend(active_extruder)) {
			manage_heater();
			lcd_update();
		}	
	...
	card.chdir(resurrectionData.nameDir);
	
	
	SERIAL_ECHOLN(resurrectionData.nameFile);
	card.openFile(resurrectionData.nameFile,true,false);
